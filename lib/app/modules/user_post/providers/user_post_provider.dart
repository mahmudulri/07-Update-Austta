...
import '../model/filter_post_modek.dart';
...
class UserPostProvider extends GetConnect {
  ...
  Future<dynamic> uploadJobPost(String title, String type, String description,String post_by,
    ...
    //       'Authorization': 'Bearer $logInToken'});
     ...
     var response =
     await post("http://172.31.120.238:80/api/save-job-post", _formData,
         headers: {
           'Authorization': 'Bearer $logInToken'});
  ...
  Future<UserJobPost?> getUserJobPost() async {
    ...
    var response = await get(
        'http://172.31.120.238:80/api/user-posts-filtering/job_post',
        headers: {'Authorization': 'Bearer $logInToken'}
    );
  ...
  Future<UserAllPost?> getUserAllPost() async {
    ...
    var response = await get(
        'http://172.31.120.238:80/api/user-posts-filtering/all',
        headers: {'Authorization': 'Bearer $logInToken'}
    );
    ...
    if (response.statusCode == 200) {
      ...
      try {
        if (response.body['status'] == 200) {
          ...
          var jsonString = response.body;
          return UserAllPost.fromJson(jsonString);
        }
      } catch (e) {
        print(e.toString());
      }
